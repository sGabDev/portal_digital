...
const { getEscola, getAnos, postEscola } = require("./getEscolas");
...
const autenticar = (req, res, next) =>
    req.session.logado ? next() : res.redirect("/");
...
app.post("/login-local", async (req, res) => {
    try {
        if (
            await global.iniciarRoboComCredenciais(req.body.cpf, req.body.senha)
        ) {
...
app.get("/api/anos", async (req, res) => {
    ...
    if (!escolaId)
        return res
            .status(400)
            .json({ success: false, message: "Escola ID é obrigatório" });
    const escolas = await getAnos(escolaId);
    res.json(escolas);
});

app.post("/api/seletor", async (req, res) => {
    const { escolaId, ano } = req.body;
    if (!escolaId || !ano) {
        return res.status(400).json({
            success: false,
            message: "Escola ID e Ano são obrigatórios",
        });
    }
    try {
        if (await postEscola(ano)) {
            await global.resetarColeta();
            return res.json({ success: true });
        }
        res.status(401).json({ success: false, message: "Falha no SEGES" });
    } catch (error) {
...
app.get("/inicio", autenticar, (req, res) =>
    res.sendFile(path.join(PUBLIC_PATH, "inicio/index.html")),
);
app.get("/diarios", autenticar, (req, res) =>
    res.sendFile(path.join(PUBLIC_PATH, "diarios/index.html")),
);
app.get("/frequencia", autenticar, (req, res) =>
    res.sendFile(path.join(PUBLIC_PATH, "frequencia/index.html")),
);
...
app.get("/diarios/start", async (req, res) => {
    ...
    processarDiario(dadosTurmas.turmas, mes, etapa, (novas) => {
        ...
        const total = dadosTurmas.turmas.reduce(
            (a, t) => a + (t.prof?.length || 0),
            0,
        );
        const done = dadosTurmas.turmas.reduce(
            (a, t) =>
                a +
                (t.prof?.filter((p) => p.statusDiario === true).length || 0),
            0,
        );
        if (done === total && total > 0)
            dadosTurmas.statusDiario = "Finalizado";
    });
    res.send("Iniciado");
});
...
app.get("/frequencia/start", async (req, res) => {
    ...
    processarFrequencia(dadosTurmas.turmas, mes, ano, (novas) => {
        ...
        const done = dadosTurmas.turmas.reduce(
            (a, t) =>
                a +
                (t.prof?.filter((p) => p.statusFrequencia === true).length ||
                    0),
            0,
        );
        if (done === total && total > 0)
            dadosTurmas.statusFrequencia = "Finalizado";
    });
    res.send("Iniciado");
});
...
async function gerarExcel(res, templateName, tipo) {
    try {
        ...
        dadosTurmas.turmas.forEach((t) => {
            t.prof?.forEach((p) => {
                const pend =
                    tipo === "diario" ? p.diarioPendente : p.frequenciaPendente;
                if (pend?.length > 0) {
                    lista.push({
                        prof: p.prof,
                        turma: t.nome,
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename=${tipo}-${date}.xlsx`,
        );
...
app.get("/diarios/download", (req, res) =>
    gerarExcel(res, "diarios.xlsx", "diario"),
);
app.get("/frequencia/download", (req, res) =>
    gerarExcel(res, "frequencia.xlsx", "frequencia"),
);

app.get("/api/reset", async (req, res) => {
    console.log("Resetando coleta");
    limparTurmas();
